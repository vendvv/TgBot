        ...
        if len(UserQueueS) != 0:
            ...
            for date in UserQueueS.keys():
                ...
                if date < earliest_date:
                   earliest_date = date
            #earliest_date = earliest_date.replace(hour= earliest_date.hour - 3) #for vercel
...
def get_text_messages(message):
    ...
    if message.text == "/create":
        ...
        bot.send_message(message.chat.id, "version 0.912")
...
def wait_until_datetime(target_datetime):
    now = datetime.now()
    if now.year == target_datetime.year and now.month == target_datetime.month and now.day == target_datetime.day and now.hour == target_datetime.hour - 3 and now.minute == target_datetime.minute: # -3 for vercel
...
def scheduled_thread():
    while True:
        ...
        target_time = target_time.replace(hour=16, minute=9, second=59) # -3 for vercel
...
def del_queue_2():# сравнивает номер первой пары в labs_tmrrw пары со значением в match, пока не придет соответствующее время, после чего удаляет первый элемент
    ...
    while labs_tmrrw:
        _time = datetime.now()
        hour = _time.hour + 3 # +3 for vercel
        minute = _time.minute
        name = labs_tmrrw[0][0]
...
def user_queue(date, name, hour, minute): #третьим параметром передать в обработчик кнопок, что очередь пользовательская
    ...
    for Suser in users:
        bot.send_message(Suser[1], f"Очередь на {name}, {date_time}\nрегистрация начнется в {start_hour+3}:{start_minute}")

    #time.sleep(300)
    usQregThread = threading.Thread(target=usQregStart(start_minute, start_hour, date_time, name)) # конечный поток # real time on vercel
    usQregThread.start()
...
def usQregStart(start_minute, start_hour, date_time, name):
    ...
    target_time = datetime(2024, 10, 10,hour=start_hour, minute=start_minute, second=0) # -3 for vercel
    print(f"target_time = {target_time}") #сюда передается правильное время на сервере vercel
